...
class Images {
    static async ImageUpload(req, res, next){
        ...
        try {
            ...
            if(findImage){
                const images = {$push: {images: [{picture: picture ,sender_id: sender_id, sender_name: user_name}]}}    
                const updateWidthNewImage =await Image.updateOne({task_id: task_id}, images)
                ...
                const imageUpload =await Image.create({title, task_id, 
                images: [{picture: picture ,sender_id: sender_id, sender_name: user_name}]})
                if(!imageUpload){
