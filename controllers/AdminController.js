...
class Admin {
    ...
    try {
      ...
      if (!admin) {
        throw new ResourseNotFoundError(
          'You do not have Admin Account with this Email: ' + email + ''
        );
      } else {
  ...
  static async CreateWorker(req, res, next) {
    ...
    try {
      ...
      res.json({
        message: 'worker is created with email av: ' + worker.email + '',
      });
    } catch (error) {
      next(error);
    }
  ...
  static async CreateClient(req, res, next) {
    ...
    try {
      ...
      res.json({
        message: 'Client is created with email: ' + client.email + '',
      });
    } catch (error) {
      next(error);
    }
  ...
  static async DeleteWorker(req, res, next) {
    ...
    try {
      ...
      res.json({ message: 'Worker is deleted with id: ' + id });
    } catch (error) {
      next(error);
    }
  }
